    ...
    return (
        <Stack direction={['column', 'row']} h='100vh'>
            <VStack w={'full'}>
                ...
                <VStack alignItems={'flex-start'} p={'5'} w={'full'} overflowY={'auto'}>
                    <Heading>
                        Sample Video
                    </Heading>
                    <Text>
            ...
            <VStack p='8' spacing={'4'} overflowY='auto' w={['full', 'xl']}>
                {
                    videoArr.map((item, index) => (
                        <Button w='full' p={2} key={index} variant={'ghost'} colorScheme='purple' onClick={() => { console.log("click"); setVideoSrc(item) }}>Video {index + 1}</Button>
                    ))
                }
            </VStack>
        </Stack>
